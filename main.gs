...
// geliştirme için, mailı değiştirebilirsin
...
function sendEmails(time) {
  ...
  const sheets = allowedSheetNames
    ...
    .filter((sheet) => sheet !== null); // ismi yanlış yazılmış veya olmayan sheet varsa listeden düşer

  sheets.forEach((sheet) => {
    const lastRow = sheet.getLastRow();

    if (lastRow < 2) return; // sadece başlık varsa veya sheet boşsa atla

    const dataRange = sheet.getRange(2, 1, lastRow - 1, sheet.getLastColumn());
    const data = dataRange.getValues();

    data.forEach((rowValues, index) => {
      const cellValue = rowValues[2]; // C sütunu, arrayde index 2
      const rowNumber = index + 2;

      if (cellValue !== time) return;

      const status = rowValues[1]  // B
      const name = rowValues[4];   // E
      const email = rowValues[5];  // F
...
// tek trigger, her 5 dakikada bir çalışacak
function checkAndSendEmails() {
  const now = new Date();
  const minutes = now.getHours() * 60 + now.getMinutes();

  // her scheduleTime için o dakikaya +5 dk içindeysek gönder
  const slots = [
    { label: "TODAY 10:00", target: 10 * 60 },
    { label: "TODAY 14:00", target: 14 * 60 },
    { label: "TODAY 18:00", target: 18 * 60 },
  ];

  for (const slot of slots) {
    if (minutes >= slot.target && minutes < slot.target + 10) {
      sendEmails(slot.label);
      return;
    }
  }
}
