...
function getSignatureBlob() {
  const blob = UrlFetchApp
    .fetch('https://raw.githubusercontent.com/kaaksoy/medipolmun-pr-otomasyon/refs/heads/main/signature.jpeg')
    .getBlob();
  return blob.setName('signature.jpeg');
}

// görünmez imza /// metinde görünmeyen x sayıda görünmez karakter üretir
function randomInvisible(length = 40) {
  const chars = ["\u200B", "\u200C", "\u200D", "\u2060", "\uFEFF"];
  let s = "";
  for (let i = 0; i < length; i++) {
    s += chars[Math.floor(Math.random() * chars.length)];
    if (i % 5 === 4) s += " "; // her 5 karakterde bir görünmez boşluk
  }
  return s;
}
...
function getSubject(status) {
  ...
  switch (status) {
    ...
    case "Dropped":
      ...
      break;
    case "Reminder":
      subject = "Payment Reminder - MedipolMUN'27";
      break;
