...
const approveTemplate = (fullName, tabName, totalPayment, iban) => {
  return `
<p>Dear ${fullName},</p>
...
const delegationApproveTemplate = (
  fullName,
  totalPayment,
  perPersonPayment,
  iban
...
<p>We are glad to inform you that your application as <strong>Delegation</strong> for MEDIPOLMUN'27 has been approved by our Secretariat. To finalize your application and receive your allocation, you need to pay the application fee of:</p>

<p><strong>PAYMENT DETAILS: ${perPersonPayment}</strong> Turkish liras.</p>
...
const staffApproveTemplate = (fullName, totalPayment, iban) => {
  return `
<p>Dear ${fullName},</p>
...
const pressApproveTemplate = (fullName, totalPayment, iban) => {
  return `
<p>Dear ${fullName},</p>
...
const receivedTemplate = (fullName, tabName) => {
  return `
<p>Dear ${fullName},</p>
...
const rejectedTemplate = (fullName, tabName) => {
  return `
<p>Dear ${fullName},</p>

<p>This email is in regards to your application for <strong>${tabName}</strong> in MEDIPOLMUN 2027. We regret to inform you that we cannot offer you that position at our conference.</p>
...
const paymentReminderTemplate = (fullName, tabName) => {
  return `
<p>Dear ${fullName},</p>
...
const paymentReceivedTemplate = (fullName, tabName) => {
  return `
<p>Dear <em>${fullName}</em>,</p>

<p>We have received your payment as <strong>${tabName}</strong> for MedipolMUN'27.</p>
...
const droppedTemplate = (fullName, tabName) => {
  return `
<p>Dear <em>${fullName}</em>,</p>

<p>We would like to inform you that your application as <strong>${tabName}</strong> was dropped because the requirements and conditions for your application have not been met.</p>
